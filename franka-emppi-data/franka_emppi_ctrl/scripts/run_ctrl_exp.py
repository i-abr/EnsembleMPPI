...
class MultModelMPPI(object):
    ...
    def __init__(self): ### since this is a ros class, I can just get away with writting this here
        ...
        self._marker_rot = euler2mat(np.array([0., -np.pi/2, 0.]))
    ...
    def update_distribution(self, state):
        ...
        self.model_probs /= np.sum(self.model_probs)
        model_prob_norm = np.linalg.norm(self.model_probs)
        ...
        for m_prob, _jnt, _pose in zip(self.model_probs, sampled_jnts, sampled_poses):
            self.viewer.add_marker(pos=_pose, 
                                    size=np.array([0.01, 0.01, 0.4]), type=const.GEOM_ARROW, label='',
                                    rgba=np.array([1.0,1.0,1.0,m_prob/model_prob_norm]),
                                    mat=self._marker_rot)
        ...
        if self.__t == self.__batch_size:
            np.save(self.script_path + self.data_path + '/model_probs_{}.npy'.format(self.__batch_no),
                            self.__dump_model_probs)
            np.save(self.script_path + self.data_path + '/sampled_jnts_{}.npy'.format(self.__batch_no), 
                    self.__dump_sampled_jnts)
            np.save(self.script_path + self.data_path + '/sampled_poses_{}.npy'.format(self.__batch_no), 
                    self.__dump_sampled_poses)
            np.save(self.script_path + self.data_path + '/mean_jnts_{}.npy'.format(self.__batch_no), 
                    self.__dump_mean_jnts)
            np.save(self.script_path + self.data_path + '/mean_poses_{}.npy'.format(self.__batch_no), 
                    self.__dump_mean_poses)
            self.__t = 0
            self.__batch_no += 1
    ...
    def update_model(self):
        alpha = 0.99 ##0.995 works well #### alpha blend the measured state and the current sim state
        v_alpha = 0.995
...
def main():
    ...
    emppi.task.randomize_param(emppi.models, emppi.handle_pos)
