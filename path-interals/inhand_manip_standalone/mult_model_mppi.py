...
class MultModelMPPI(object):
    ...
    def __init__(self, models, task, terminal_cost, frame_skip=1, horizon=10, num_trajectories=5, noise=0.2, lam=10.0):
        ...
        self.num_sensors = len(self.data[0].sensordata[:]) ### I dont think i really need this now
    ...
    def __call__(self, state, predict_measurements=False): ### call to compute the control and return it
        ...
        self.mean_actions[-1] = 0*np.random.normal(0., self.noise, size=(self.num_actions,))
        #print('shifting actions takes {} s'.format(time.time() - start_time))
        # plt.figure(1)
        # plt.clf()
        #
        # plt.plot(self.mean_actions)
        # self.mean_actions = savgol_filter(self.mean_actions, len(self.mean_actions)-1, 2, axis=0)
        # plt.figure(2)
        # plt.clf()
        # plt.plot(self.mean_actions)
        # plt.pause(0.001)
        ...
        for sim in self.pool.sims: ### reset each of the simulators initial state
            sim.set_state(state)
        #print('setting the state takes {}s'.format(time.time() - start_time))


        #start_time = time.time()
        self.delta = np.random.normal(0., self.noise, size=self.delta.shape) ### create the random action perturbation
        #print('generating random control samples takes {} s'.format(time.time() - start_time))
        ...
        for t in range(self.horizon): ### for each time step
            ...
            for k, data in enumerate(self.data): ### do each parameter fixing
                ...
                if t == self.horizon-1:
                    self.sk[k,t] += self.terminal_cost(data) #* (0.99**(self.horizon))

            # for _ in range(self.frame_skip):
            self.pool.step() ### step the simulator

        #print('The simulation loop took : {}s'.format(time.time() - start_time))
        ...
        for t in range(self.horizon):### loop through each action and do the following
            ...
            _w = self.model_probs * exp(-self.sk[:,t]/self.lam) + 1e-4### this should be of size no_trajectories
        ...
        #print('updating the ctrls toop : {}s'.format(time.time() - start_time))
    ...
    def update_distribution(self, true_measurements, state, ctrl):
        ...
        for i, (data, model_prob) in enumerate(zip(self.data, self.model_probs)):
            ...
            # TODO comment out for mass deviation + update shared.xml (remove joint vel + joint pose)
            #likelihood = np.prod(multivariate_normal.pdf(diff, 0.,0.0000001))
            likelihood = np.sum(multivariate_normal.logpdf(diff, 0., 1.0))
            likelihoods.append(likelihood)

        likelihoods = np.array(likelihoods)
        ## condition the likeliness
        likelihoods -= np.max(likelihoods)
        self.model_probs = self.model_probs * np.exp(likelihoods)
