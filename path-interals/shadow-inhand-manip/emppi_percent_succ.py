...
def randomize_param(models):
    ...
    for model in models:
        ...
        model.body_mass[obj_bid] = np.clip(np.random.normal(mean_mass, 0.5), 0.001, np.inf)
...
def main():
    ...
    cnt = 0
    performance_data = {'successes' : [], 'time2success' : [], 'stats': []}
    ...
    while cnt < max_attempts:
        ...
        for t in range(final_time):
            ...
            performance_data['stats'].append(stats)
        ...
        if cnt % 2 == 0:
            print('saving at :', cnt)
            path = './data/percent_success/emppi/'
